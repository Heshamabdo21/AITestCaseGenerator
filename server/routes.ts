...
For each test case, provide the following in JSON format:
{
  ...
  // Export selected test cases to Excel (for user story-specific exports)
  app.post("/api/test-cases/export", async (req, res) => {
    try {
      ...
      if (testCaseIds && Array.isArray(testCaseIds)) {
        testCases = await activeStorage.getTestCasesByIds(testCaseIds);
      } else {
        testCases = await activeStorage.getTestCases();
      }
      ...
      res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
      res.setHeader('Content-Disposition', 'attachment; filename="test_cases_selected.xlsx"');
      res.send(excelBuffer);
    } catch (error: any) {
      console.error('Excel export error:', error);
      res.status(500).json({ message: `Failed to export test cases: ${error.message}` });
